...
pub async fn create_task(Extension(database): Extension<PgPool>, Json(request_task): Json<RequestTask>) {
    ...
    match insert_task(&database, query, &request_task).await {
        Ok(task) => {
...
async fn insert_task(database: &PgPool, query: &str, request_task: &RequestTask) -> Result<RequestTask, sqlx::Error> {
    let task = sqlx::query_as::<_, RequestTask>(query)
        .bind(&request_task.priority)      // Bind the priority field
        .bind(&request_task.title)         // Bind the title field
        .bind(&request_task.description)   // Bind the description field
